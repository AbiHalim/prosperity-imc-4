...
class Trader(RoundThreeTrader):
    ...
    def mr_hydrogel_pack(self):
        ...
        if pos_diff > 0:
            qty = min(pos_diff, self.max_buy_orders_left(product))
            if qty > 0:
                # Use best_ask to "take" liquidity
                self.send_buy_order(product, best_ask, qty, f"MR Buy (Take) tgt={target_pos}")
                
        elif pos_diff < 0:
            qty = min(-pos_diff, self.max_sell_orders_left(product))
            if qty > 0:
                # Use best_bid to "take" liquidity
                self.send_sell_order(product, best_bid, qty, f"MR Sell (Take) tgt={target_pos}")
