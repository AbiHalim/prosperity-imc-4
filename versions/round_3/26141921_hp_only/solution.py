...
class Trader(RoundThreeTrader):
    ...
    def hybrid_hydrogel_pack(self):
        ...
        if 'hp_fast_ema' not in self.data:
            self.data['hp_fast_ema'] = mid
            self.data['hp_slow_ema'] = mid
            
        fast_ema = 0.1 * mid + 0.9 * self.data['hp_fast_ema']
        slow_ema = 0.005 * mid + 0.995 * self.data['hp_slow_ema']
        
        self.data['hp_fast_ema'] = fast_ema
        self.data['hp_slow_ema'] = slow_ema
        ...
        # --- ANTI-DRAWDOWN: TREND FILTER ---
        # If the market is violently crashing (trend < -5), don't buy! Wait for it to stabilize.
        if trend < -5.0:
            our_bid = -1  # Disable bidding
        # If the market is violently rocketing (trend > 5), don't sell! Wait for it to stabilize.
        if trend > 5.0:
            our_ask = 9999999  # Disable asking
