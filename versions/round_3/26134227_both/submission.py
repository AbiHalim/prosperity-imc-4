...
class Trader(RoundThreeTrader):
    ...
    def mm_velvetfruit_extract(self):
        ...
        # 1. Calculate Implied Spot from ATM Options
        T = self._current_tte()
        implied_spots = []
        ...
        for K in [4500, 5000, 5100, 5200]:
            ...
            # 0.24 is the rock-solid ATM implied vol we verified previously
        ...
        if not implied_spots:
            # Fallback if no option data
            true_spot = vfx_mid
        else:
            # Average the implied spots
            true_spot = sum(implied_spots) / len(implied_spots)
            
        # 2. Predictive Quotes
        # We quote around true_spot instead of vfx_mid!
        BASE_HALF_SPREAD = 3
