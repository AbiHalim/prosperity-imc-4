...
class Trader(RoundThreeTrader):
    ...
    def hybrid_hydrogel_pack(self):
        ...
        if pos > 0:
            ...
            # Increased skew factor from 0.5 to 1.5. This drastically reduces drawdowns 
            # during a crash by buying much smaller amounts as the price drops!
            our_bid = FAIR_VALUE - ENTRY_DEV - (pos * 1.5)
            
        elif pos < 0:
            # We are SHORT.
            # We want to exit (buy) when price reverts close to fair value.
            our_bid = FAIR_VALUE + EXIT_DEV
            our_ask = FAIR_VALUE + ENTRY_DEV + (-pos * 1.5)
