...
class Trader(RoundThreeTrader):
    ...
    def hybrid_hydrogel_pack(self):
        ...
        mid = (best_bid + best_ask) / 2.0
        # 1. Adaptive Fair Value
        fair, std = self._update_ema_var(mid, "hp_ema", "hp_var", 0.05)
        
        # 2. Trend Filter (Don't catch a falling knife)
        ...
        # 3. Dynamic Quotes
        BASE_HALF_SPREAD = max(2, math.floor(std * 1.5))
        
        pos = self.position.get(product, 0)
        
        # Convex Skew: 
        # Keeps quotes tight when position is small, but violently widens them when maxing out.
        inv_skew = -pos * abs(pos) * 0.0005

        our_bid = math.floor(fair - BASE_HALF_SPREAD + inv_skew)
        our_ask = math.ceil(fair + BASE_HALF_SPREAD + inv_skew)

        # --- ANTI-DRAWDOWN: TREND FILTER ---
        if trend < -3.0:
            our_bid = -1  # Market crashing, do not buy!
        if trend > 3.0:
            our_ask = 9999999  # Market rocketing, do not sell!
