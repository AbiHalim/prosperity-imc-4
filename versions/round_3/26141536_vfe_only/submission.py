...
class Trader(RoundThreeTrader):
    ...
    def mm_velvetfruit_extract(self):
        ...
        T = (self.data['T_days_left'] - self.data['elapsed_ticks'] / 10000.0) / 365.0
        T = max(1e-6, T)
        ...
        for K in [4500, 5000, 5100, 5200, 5300, 5400, 5500]:
            ...
            if delta > 0.2:
                # Store tuple of (Implied Spot, Delta) for weighted averaging
                implied_spots.append((vfx_mid + (opt_mid - c_bs) / delta, delta))
                
        if not implied_spots:
            true_spot = vfx_mid
        else:
            # Delta-weighted average: Options closer to ATM (higher delta) have cleaner signals
            true_spot = sum(S * d for S, d in implied_spots) / sum(d for S, d in implied_spots)
            
        diff = true_spot - vfx_mid

        # 3. Dynamic Spread & Convex Inventory Skew
        if abs(diff) > 4.5:
            BASE_HALF_SPREAD = 0
        elif abs(diff) > 2.5:
            BASE_HALF_SPREAD = 1
        ...
        # Convex Skew: Loose at small positions to ride trends, violent at max position!
        # pos=50 -> 0.5 ticks, pos=100 -> 2.0 ticks, pos=200 -> 8.0 ticks
        inv_skew = -pos * abs(pos) * 0.0002
        
        our_bid = math.floor(true_spot - BASE_HALF_SPREAD + inv_skew)
        our_ask = math.ceil(true_spot + BASE_HALF_SPREAD + inv_skew)

        # 4. Aggressive Take Profit (Anti-Drawdown)
        # If we built a large long position but the options market flips bearish, dump it instantly!
        if pos > 50 and diff < -1.0:
            our_ask = vfx_bid  # Cross the spread to sell
        # If we built a large short position but the options market flips bullish, cover instantly!
        elif pos < -50 and diff > 1.0:
            our_bid = vfx_ask  # Cross the spread to buy
    ...
    def hybrid_hydrogel_pack(self):
        ...
        # Directional Mean Reversion parameters
        FAIR_VALUE = 9991.0
        ENTRY_DEV = 8   # Lowered from 14 to 8 to trigger way more trades!
        EXIT_DEV = 2    # Exit when price reverts to within 2 ticks of mean

        # Default quotes if flat:
        our_bid = FAIR_VALUE - ENTRY_DEV
        our_ask = FAIR_VALUE + ENTRY_DEV

        pos = self.position.get(product, 0)

        if pos > 0:
            # We are LONG. 
            # We want to exit (sell) when price reverts close to fair value.
            our_ask = FAIR_VALUE - EXIT_DEV
            # Scale down our bid significantly to avoid buying too much if it keeps dropping
            # A factor of 0.5 means at pos=200, our bid is pushed down by 100 ticks.
            our_bid = FAIR_VALUE - ENTRY_DEV - (pos * 0.5)
            
        elif pos < 0:
            # We are SHORT.
            # We want to exit (buy) when price reverts close to fair value.
            our_bid = FAIR_VALUE + EXIT_DEV
            # Scale up our ask significantly to avoid selling too much if it keeps rising
            our_ask = FAIR_VALUE + ENTRY_DEV + (-pos * 0.5)

        our_bid = math.floor(our_bid)
        our_ask = math.ceil(our_ask)
        ...
        if buy_qty > 0:
            self.send_buy_order(product, our_bid, buy_qty,
                                msg=f"HP Directional bid={our_bid} pos={pos}")

        sell_qty = self.max_sell_orders_left(product)
        if sell_qty > 0:
            self.send_sell_order(product, our_ask, sell_qty,
                                 msg=f"HP Directional ask={our_ask} pos={pos}")
