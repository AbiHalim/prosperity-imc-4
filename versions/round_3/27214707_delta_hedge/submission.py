...
class Trader(RoundThreeTrader):
    ...
    def mm_velvetfruit_extract(self):
        ...
        # When the signal is weak, quote a wider spread to capture Mark 55.
        # When the signal is strong, tighten the spread but NEVER to 0 to avoid
        # crossing the spread and paying Smart Money (Mark 01, Mark 14).
        if abs(diff) > 4.5:
            BASE_HALF_SPREAD = 1
        elif abs(diff) > 2.5:
            BASE_HALF_SPREAD = 2
        ...
        vfx_pos = self.position.get(product, 0)
        vev_4000_pos = self.position.get("VEV_4000", 0)
        
        # DELTA HEDGING: VEV_4000 is deeply ITM, so its delta is ~1.0.
        # We must sum our VFX inventory and our VEV_4000 inventory to find our true directional exposure.
        net_delta = vfx_pos + vev_4000_pos
        
        # Use class-level skew (0.05) on the NET DELTA to hedge our options exposure
        inv_skew = -net_delta * self.VFX_SKEW_PER_UNIT
        ...
        if buy_qty > 0:
            self.send_buy_order(product, our_bid, buy_qty,
                                msg=f"VFX Pred bid={our_bid} implied={true_spot:.1f} net_delta={net_delta}")

        sell_qty = self.max_sell_orders_left(product)
        if sell_qty > 0:
            self.send_sell_order(product, our_ask, sell_qty,
                                 msg=f"VFX Pred ask={our_ask} implied={true_spot:.1f} net_delta={net_delta}")
    ...
    def hybrid_hydrogel_pack(self):
        ...
        and using dynamic EMA for fair value.
        """
        self._market_make(
            "HYDROGEL_PACK",
