...
class Trader(RoundThreeTrader):
    ...
    def mm_velvetfruit_extract(self):
        ...
        pos = self.position.get(product, 0)
        # Reduced skew from 0.05 to 0.02. This allows the bot to take larger positions
        # (up to max limits) when a strong trend is predicted, rather than exiting early.
        inv_skew = -pos * 0.02
        
        our_bid = math.floor(true_spot - BASE_HALF_SPREAD + inv_skew)
        our_ask = math.ceil(true_spot + BASE_HALF_SPREAD + inv_skew)
