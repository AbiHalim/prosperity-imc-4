...
class Trader(RoundThreeTrader):
    ...
    def mm_hydrogel_pack(self):
        ...
        if pos > 0:
            ...
            # Scale down our bid to avoid buying too much if it keeps dropping
            our_bid = FAIR_VALUE - ENTRY_DEV - (pos * 0.2)
            
        elif pos < 0:
            # We are SHORT.
            # We want to exit (buy) when price reverts close to fair value.
            our_bid = FAIR_VALUE + EXIT_DEV
            # Scale up our ask to avoid selling too much if it keeps rising
            our_ask = FAIR_VALUE + ENTRY_DEV + (-pos * 0.2)
