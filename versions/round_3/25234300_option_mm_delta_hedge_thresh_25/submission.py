...
class RoundThreeTrader(NormalTrader):
    ...
    OPT_MM_BASE_HALF_SPREAD = 5    # 5-tick spread: balance between fill rate and income
    OPT_MM_SIZE             = 10   # larger size per fill → more income per counterparty
    ...
    # IMPORTANT: set high to reduce crossing costs — each hedge costs the
    # VFX bid-ask spread. With DELTA_HEDGE_THRESH=5 and 6 active strikes,
    # we rehedge constantly, eating all spread income. 25 gives a good
    # balance between delta exposure and hedge friction cost.
    DELTA_HEDGE_THRESH  = 25.0
