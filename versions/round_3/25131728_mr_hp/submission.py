...
class Trader(RoundThreeTrader):
    def mr_vfe(self):
        ...
        if "vfe_ema" not in self.data:
            self.data["vfe_ema"] = mid_price
            self.data["vfe_var"] = 3.0  # Initial guess for variance
            
        alpha = 0.3 # Fast rolling factor (higher = faster adaptation)
        ema = self.data["vfe_ema"]
        var = self.data["vfe_var"]
        ...
        # Save state
        self.data["vfe_ema"] = new_ema
        self.data["vfe_var"] = new_var
    ...
    def mr_hp(self):
        ...
        # 3. Trade deviations
        # Target position is based on a fixed amount deviation from EMA
        fixed_threshold = 0.5  # Configure this fixed amount
        limit = self.pos_limits[product]
