...
class Trader(RoundThreeTrader):
    ...
    def mm_hydrogel_pack(self):
        """
        Market make HYDROGEL_PACK using the optimized mean-reversion strategy.
        Data analysis shows HP is highly mean-reverting to 9991, with a half-life
        of 189 ticks. It spends ~44% of its time >20 ticks away from 9991.
        We anchor strictly to 9991, and apply a strong mean-reversion skew to
        our quotes to lean heavily into the expected reversal.
        """
        ...
        # Core parameters from data analysis
        FAIR_VALUE = 9991.0
        BASE_HALF_SPREAD = 3         # tighter than before to get filled
        MR_SKEW_FACTOR = 0.5         # aggressively lean into deviation
        INV_SKEW_PER_UNIT = 0.05
        EXTREME_THRESH = 25          # threshold to cross the spread aggressively

        mid = (best_bid + best_ask) / 2.0
        deviation = mid - FAIR_VALUE

        # 1. Aggressive Market Taking (Fade extremes)
        # If price spikes > 25 ticks away from 9991, cross the spread to take it
        if deviation > EXTREME_THRESH:
            # Price is very high → sell aggressively
            self.match_sell_with_buy(product, round(FAIR_VALUE + 5),
                                     msg=f"HP extreme-fade sell (dev={deviation:.1f})")
        elif deviation < -EXTREME_THRESH:
            # Price is very low → buy aggressively
            self.match_buy_with_sell(product, round(FAIR_VALUE - 5),
                                     msg=f"HP extreme-fade buy (dev={deviation:.1f})")

        # 2. Passive Market Making with MR Skew
        # If mid > 9991 (dev > 0), mr_skew is negative → both quotes shift down.
        # This makes our ask very competitive (to sell the high) and our bid deep.
        mr_skew = -deviation * MR_SKEW_FACTOR

        pos = self.position.get(product, 0)
        inv_skew = -pos * INV_SKEW_PER_UNIT

        total_skew = mr_skew + inv_skew
        
        # Quote around the TRUE fair value, not the mid price!
        our_bid = math.floor(FAIR_VALUE - BASE_HALF_SPREAD + total_skew)
        our_ask = math.ceil(FAIR_VALUE  + BASE_HALF_SPREAD + total_skew)
        ...
        if buy_qty > 0:
            self.send_buy_order(product, our_bid, buy_qty,
                                msg=f"HP MM bid={our_bid} dev={deviation:.1f} skew={total_skew:.1f}")

        sell_qty = self.max_sell_orders_left(product)
        if sell_qty > 0:
            self.send_sell_order(product, our_ask, sell_qty,
                                 msg=f"HP MM ask={our_ask} dev={deviation:.1f} skew={total_skew:.1f}")
