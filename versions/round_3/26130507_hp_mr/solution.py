...
class Trader(RoundThreeTrader):
    ...
    def mm_hydrogel_pack(self):
        """
        Directional Mean-Reversion Strategy for HYDROGEL_PACK.
        Instead of market making around the mean (which causes churning),
        we only enter positions when the price deviates significantly from 9991,
        and we hold until the price reverts back to the mean.
        """
        ...
        FAIR_VALUE = 9991.0
        ENTRY_DEV = 14  # Ticks away from fair to ENTER a position
        EXIT_DEV = 2    # Ticks away from fair to EXIT a position
