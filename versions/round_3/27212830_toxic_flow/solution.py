...
class Trader(RoundThreeTrader):
    ...
    def _market_make(self, product: str,
        ...
        # Apply adjustment from toxic flow / informed trading
        fair += fair_value_adjust
        ...
        # --- 4. Quoted prices ---
        # Calculate ideal prices based on fair value, spread and skew
        our_bid_ideal = math.floor(fair - half_spread + skew)
        our_ask_ideal = math.ceil(fair + half_spread + skew)

        # We are liquidity providers to capture Mark's spread crossing.
        # NEVER cross the spread ourselves!
        max_bid = best_ask - 1 if best_ask is not None else float('inf')
        min_ask = best_bid + 1 if best_bid is not None else float('-inf')

        # Try to penny the market, but don't quote worse than our ideal,
        # and strictly bound by the spread limits.
        our_bid = min(our_bid_ideal, best_bid + 1 if best_bid is not None else our_bid_ideal)
        our_bid = min(our_bid, max_bid)

        our_ask = max(our_ask_ideal, best_ask - 1 if best_ask is not None else our_ask_ideal)
        our_ask = max(our_ask, min_ask)
    ...
    def mm_velvetfruit_extract(self):
        """
        Predictive Market Making for VELVETFRUIT_EXTRACT.
        Uses the implied spot price from the VEV options market (which leads spot)
        as our "True Fair Value" for quoting VFX. This avoids adverse selection
        from smart flow and predicts short-term directional trends.
        """
        product = "VELVETFRUIT_EXTRACT"
        vfx_bid = self.best_bid(product)
        vfx_ask = self.best_ask(product)
        if vfx_bid is None or vfx_ask is None:
            return
            
        vfx_mid = (vfx_bid + vfx_ask) / 2.0
        
        # 1. Infer True TTE on startup (Retry if order book is empty on tick 0)
        if 'elapsed_ticks' not in self.data:
            self.data['elapsed_ticks'] = 0
        else:
            self.data['elapsed_ticks'] += 1
            
        if 'T_days_left' not in self.data:
            # Infer whether it's Day 0, 1, or 2 by checking ATM option pricing
            # We know IV is ~24%. We see which T_days makes BS price closest to Market.
            best_diff = float('inf')
            best_T = 8.0
            opt_bid_5000 = self.best_bid("VEV_5000")
            opt_ask_5000 = self.best_ask("VEV_5000")
            
            if opt_bid_5000 and opt_ask_5000:
                c_mkt = (opt_bid_5000 + opt_ask_5000) / 2.0
                # Scan all possible integer days to expiry (up to 10)
                for test_T in [float(i) for i in range(1, 11)]:
                    c_bs, _ = bs_call_price_and_delta(vfx_mid, 5000, test_T / 365.0, 0.24)
                    if abs(c_bs - c_mkt) < best_diff:
                        best_diff = abs(c_bs - c_mkt)
                        best_T = test_T
                
                self.data['T_days_left'] = best_T

        # Fallback to 8.0 if we haven't successfully inferred it yet
        T_days = self.data.get('T_days_left', 8.0)
        T = (T_days - self.data['elapsed_ticks'] / 10000.0) / 365.0
        T = max(1e-6, T)

        # 2. Calculate Implied Spot from ATM Options
        implied_spots = []
        
        # Check a few strikes around ATM to get a stable signal
        # We include up to 5500 because Spot has drifted up to ~5265.
        for K in [4500, 5000, 5100, 5200, 5300, 5400, 5500]:
            ...
            opt_mid = (opt_bid + opt_ask) / 2.0
            c_bs, delta = bs_call_price_and_delta(vfx_mid, K, T, 0.24)
            
            # Approximate implied spot: S_implied = S + (C_mkt - C_bs) / Delta
            # We enforce a strict Delta > 0.2 to filter out OTM noise
            if delta > 0.2:
                implied_spots.append(vfx_mid + (opt_mid - c_bs) / delta)
                
        if not implied_spots:
            true_spot = vfx_mid
        else:
            true_spot = sum(implied_spots) / len(implied_spots)
            
        # Track Toxic Flow from Informed (Mark 67) and Reverse-Informed (Mark 49)
        if 'vfx_toxic_flow' not in self.data:
            self.data['vfx_toxic_flow'] = 0.0
            
        self.data['vfx_toxic_flow'] *= 0.90
        
        for t in self.market_trades.get(product, []):
            if t.buyer == "Mark 67":
                self.data['vfx_toxic_flow'] += t.quantity
            elif t.seller == "Mark 49":
                self.data['vfx_toxic_flow'] += t.quantity
            elif t.buyer == "Mark 49":
                self.data['vfx_toxic_flow'] -= t.quantity
                
        # Each unit of flow moves the true spot by 0.2 ticks
        true_spot += self.data['vfx_toxic_flow'] * 0.2
            
        diff = true_spot - vfx_mid

        # 3. Dynamic Spread & Inventory Skew
        # Demand 1 tick of edge so we reliably penny Mark 14 and capture Mark 55.
        BASE_HALF_SPREAD = 1
            
        pos = self.position.get(product, 0)
        # Use class-level skew (0.05) to prevent catastrophic drawdowns/falling knives
        inv_skew = -pos * self.VFX_SKEW_PER_UNIT
        
        our_bid_ideal = math.floor(true_spot - BASE_HALF_SPREAD + inv_skew)
        our_ask_ideal = math.ceil(true_spot + BASE_HALF_SPREAD + inv_skew)
        
        max_bid = vfx_ask - 1 if vfx_ask is not None else float('inf')
        min_ask = vfx_bid + 1 if vfx_bid is not None else float('-inf')

        our_bid = min(our_bid_ideal, vfx_bid + 1 if vfx_bid is not None else our_bid_ideal)
        our_bid = min(our_bid, max_bid)

        our_ask = max(our_ask_ideal, vfx_ask - 1 if vfx_ask is not None else our_ask_ideal)
        our_ask = max(our_ask, min_ask)
        ...
        if buy_qty > 0:
            self.send_buy_order(product, our_bid, buy_qty,
                                msg=f"VFX Pred bid={our_bid} implied={true_spot:.1f} pos={pos}")

        sell_qty = self.max_sell_orders_left(product)
        if sell_qty > 0:
            self.send_sell_order(product, our_ask, sell_qty,
                                 msg=f"VFX Pred ask={our_ask} implied={true_spot:.1f} pos={pos}")

    def hybrid_hydrogel_pack(self):
        """
        Market Making Strategy for HYDROGEL_PACK.
        Captures the wide spread from Mark 38 (Dumb Money) by pennying the market
        and using dynamic EMA for fair value. Also adjusts for toxic flow.
        """
        product = "HYDROGEL_PACK"
        
        # Track Toxic Flow from Informed (Mark 14) and Reverse-Informed (Mark 38)
        if 'hp_toxic_flow' not in self.data:
            self.data['hp_toxic_flow'] = 0.0
            
        self.data['hp_toxic_flow'] *= 0.90
        
        for t in self.market_trades.get(product, []):
            if t.buyer == "Mark 14":
                self.data['hp_toxic_flow'] += t.quantity
        ...
        fair_adjust = self.data['hp_toxic_flow'] * 0.05  # Slight adjustment for HP

        self._market_make(
            product,
            self.HP_BASE_HALF_SPREAD,
            self.HP_VOL_MULT,
            self.HP_SKEW_PER_UNIT,
            self.HP_EMA_ALPHA,
            "hp_ema",
            "hp_var",
            fair_value_adjust=fair_adjust
        )

    def mm_vev_4000(self):
        """
        Market Making Strategy for VEV_4000.
        Captures the huge spread from Mark 38 by quoting around the intrinsic value.
        Mark 38 buys at Intrinsic + 10 and sells at Intrinsic - 10.
        We quote slightly inside that to penny Mark 14.
        """
        product = "VEV_4000"
        ...
        # Deep ITM call option fair value is basically intrinsic value
        fair_value = vfx_mid - 4000
        
        pos = self.position.get(product, 0)
        
        # Mark 38 quotes are roughly +/- 10 ticks from intrinsic.
        # We quote +/- 8 ticks to reliably penny Mark 14.
        BASE_HALF_SPREAD = 8
        skew = -pos * 0.05
        
        our_bid_ideal = math.floor(fair_value - BASE_HALF_SPREAD + skew)
        our_ask_ideal = math.ceil(fair_value + BASE_HALF_SPREAD + skew)
        
        best_bid = self.best_bid(product)
        best_ask = self.best_ask(product)
        
        max_bid = best_ask - 1 if best_ask else float('inf')
        min_ask = best_bid + 1 if best_bid else float('-inf')

        if best_bid is not None:
            our_bid = min(our_bid_ideal, best_bid + 1)
        else:
            our_bid = our_bid_ideal
            
        if best_ask is not None:
            our_ask = max(our_ask_ideal, best_ask - 1)
        else:
            our_ask = our_ask_ideal
        ...
        if buy_qty > 0:
            self.send_buy_order(product, our_bid, buy_qty,
                                msg=f"VEV_4000 MM bid={our_bid} fair={fair_value:.1f} pos={pos}")

        sell_qty = self.max_sell_orders_left(product)
        if sell_qty > 0:
            self.send_sell_order(product, our_ask, sell_qty,
                                 msg=f"VEV_4000 MM ask={our_ask} fair={fair_value:.1f} pos={pos}")
