...
class Trader(RoundThreeTrader):
    ...
    def mm_velvetfruit_extract(self):
        ...
        # When the signal is weak, quote a normal spread of 2 to "penny" the market.
        # When the signal is strong, tighten the spread to 1 or 0 to aggressively take liquidity!
        if abs(diff) > 4.5:
            BASE_HALF_SPREAD = 0
        elif abs(diff) > 2.5:
            BASE_HALF_SPREAD = 1
    ...
    def hybrid_hydrogel_pack(self):
        ...
        mid = (best_bid + best_ask) / 2.0
        # 1. Macro Fair Value (Very Slow EMA)
        # alpha=0.0005 gives a half-life of ~1386 ticks.
        # This acts as a stable anchor for local mean-reversion, but slowly adapts to macro trends!
        if 'hp_macro_ema' not in self.data:
            self.data['hp_macro_ema'] = mid
            
        fair = 0.0005 * mid + 0.9995 * self.data['hp_macro_ema']
        self.data['hp_macro_ema'] = fair

        # 2. Directional Mean Reversion Parameters
        # HP median spread is 16. Setting ENTRY=8 perfectly pennies the market!
        ENTRY_DEV = 8
        EXIT_DEV = 2
        
        pos = self.position.get(product, 0)
        
        # Convex Skew: pos=50 -> 1.25, pos=100 -> 5.0, pos=150 -> 11.25
        # This prevents us from immediately maxing out during a crash.
        inv_skew = -pos * abs(pos) * 0.0005

        if pos > 0:
            # We are LONG. Exit when price reverts close to fair value.
            our_ask = fair - EXIT_DEV
            our_bid = fair - ENTRY_DEV + inv_skew
        elif pos < 0:
            # We are SHORT. Exit when price reverts close to fair value.
            our_bid = fair + EXIT_DEV
            our_ask = fair + ENTRY_DEV + inv_skew
        else:
            our_bid = fair - ENTRY_DEV
            our_ask = fair + ENTRY_DEV
        ...
        buy_qty = self.max_buy_orders_left(product)
        if buy_qty > 0 and our_bid > 0:
            self.send_buy_order(product, our_bid, buy_qty,
                                msg=f"HP MM bid={our_bid} fair={fair:.1f} pos={pos}")

        sell_qty = self.max_sell_orders_left(product)
        if sell_qty > 0 and our_ask < 9999999:
            self.send_sell_order(product, our_ask, sell_qty,
                                 msg=f"HP MM ask={our_ask} fair={fair:.1f} pos={pos}")
