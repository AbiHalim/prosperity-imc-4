...
class RoundThreeTrader(NormalTrader):
    ...
    VFX_BASE_HALF_SPREAD   = 0    # 0 = quote AT the existing market spread;
                                    # 1 = step back 1 tick on each side (wider)
    ...
    VFX_HIGH_VOL_STEPBACK  = 1     # step back this many ticks on each side in high vol
    VFX_POS_LIMIT          = 200
...
class Trader(RoundThreeTrader):
    ...
    def mm_velvetfruit_extract(self):
        ...
        worst_bid = self.worst_bid("VELVETFRUIT_EXTRACT")
        worst_ask = self.worst_ask("VELVETFRUIT_EXTRACT")
        if worst_bid is None or worst_ask is None:
            return

        mid = (best_bid + best_ask) / 2.0
        # mid = (worst_bid + worst_ask) / 2.0
        ...
        # Step back from the market in high vol (accept less fills but wider spread)
        vol_stepback = self.VFX_HIGH_VOL_STEPBACK if vol_ratio > self.HIGH_VOL_RATIO else 0
        ...
        # --- Final quotes ---
        # Anchor to existing best_bid / best_ask so we are INSIDE the market.
        # Quoting mid ± half_spread puts us OUTSIDE the existing spread and
        # no counterparty will ever cross us. By starting from best_bid /
        # best_ask and applying integer shifts, we guarantee competitiveness.
        skew_int = int(round(mr_skew + inv_skew))
        our_bid = best_bid - vol_stepback + skew_int
        our_ask = best_ask + vol_stepback + skew_int
        if our_ask <= our_bid:
            our_ask = our_bid + 1
