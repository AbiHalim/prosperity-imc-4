...
"""
First try to trade Intarian Pepper Root
Fair value calculated based on linear regression
y = 0.1x + 10000 (assuming timestamp //100 )
"""
...
class Trader(Round1Trader):
    def trade_intarian_pepper_root(self):
        fair_price = self.ts * 0.1 + 10000

        # Match with every order that's better than the fair price.
        self.match_buy_with_sell("INTARIAN_PEPPER_ROOT", fair_price)
        self.match_sell_with_buy("INTARIAN_PEPPER_ROOT", fair_price)
