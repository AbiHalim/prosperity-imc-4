...
class NormalTrader:
    def load_state(self, state: TradingState):
        ...
        # REDUCED VISIBILITY: Randomly ignore 20% of orders
        self.buy_orders = {symbol: [] for symbol in self.symbol_map.keys()}
        self.sell_orders = {symbol: [] for symbol in self.symbol_map.keys()}
        ...
        for symbol, order_depth in state.order_depths.items():
            ...
            for price, vol in order_depth.buy_orders.items():
                if random.random() < 0.8:
                    bids.append((round(price), round(vol)))
            self.buy_orders[symbol] = sorted(bids, key=lambda t: t[0], reverse=True)

            # Filter Asks
            asks = []
            for price, vol in order_depth.sell_orders.items():
                if random.random() < 0.8:
                    asks.append((round(price), -round(vol)))
            self.sell_orders[symbol] = sorted(asks, key=lambda t: t[0])
...
class Trader(RoundOneTrader):
    def trade_ash_coated_osmium(self):
        ...
        self.match_buy_with_sell(product, fair_price - CONFIG["of"])
        self.match_sell_with_buy(product, fair_price + CONFIG["of"])

        # 2. Market make around the fair price, but turn off one side
        # if position is too skewed.
        best_bid = self.best_bid(product)
        # if best_bid is not None and best_bid < fair_price - CONFIG["of"] \
        ...
        best_ask = self.best_ask(product)
        # if best_ask is not None and best_ask > fair_price + CONFIG["of"] \
    ...
    def _run(self):
        ...
        # Set starting price if not already set (applies to both products;
        # we expect osmium to stay around 10000 while pepper root goes up
        # about 0.1 per tick)
        for symbol in self.symbol_map:
            if symbol not in self.data["starting_prices"]:
                ...
                if worst_ask is not None and worst_bid is not None:
                    ...
                    # Round to nearest 100 to snap to the day's
                    # theoretical starting point
                    # 100 is chosen because we expect the pepper root to
                    # start at a multiple of 1000 ticks since last time
